...
| `app/admin/stores/page.tsx` | Site-wide store list: inline name / Play! id edit, counts, delete, create form, and each store's owner/admin roster (site admins pick the role) | `await requireAdmin()` |
| `app/stores/[slug]/admin/page.tsx` | Store-scoped console (PL-17): store details, prize budget defaults per player (PL-29, `StorePrizeDefaultsForm` → `store_prize_defaults`, see `docs/features/events.md` "Prize budget"), its leagues + create league (store fixed), its events + create event (store fixed, paste box included), admin roster | `isStoreAdmin(store.id)` → redirects to `/` if false |
...
- `store-roster.tsx` — owner/admin list of a store with remove buttons and the add form (`useActionState` over `addStoreAdminAction`); `canManage` (owners) and `canSetOwner` (site admins) decide what renders, the RPCs enforce it.
- `store-details-form.tsx` — name + Play! League ID of one store, posting `updateStoreAction`.
...
Exposes `getUser`, `getProfile` (both `cache`-memoized), `requireUser` (redirect `/login`), and `requireAdmin` — redirects to `/login` if unauthenticated, `/` if `profile.is_admin` is false. `requireAdmin` is the standard guard called at the top of every `app/admin/*` page. Non-site-scoped admin checks use `lib/leagues.ts` / `lib/events.ts` / `lib/stores.ts` helpers (`isLeagueAdmin`, `isEventAdmin`, `isStoreAdmin`, `isStoreOwner`) which, after a site-admin short-circuit, call the SQL helpers `is_league_admin` / `is_event_admin` / `is_store_admin` / `is_store_owner` over RPC — the same functions the RLS policies and RPCs use. Since 0046 a store admin is an admin (and owner) of every league and event of the store.
...
- `0046_store_admins.sql` — `store_admins` (owner|admin per store), `is_store_admin` / `is_store_owner`, `add_store_admin` / `remove_store_admin` RPCs, and the redefinition of `is_league_admin` / `is_league_owner` / `is_event_admin` / `is_event_owner` to include the store's admins; `create_league` / `create_event` accept an admin of the target store. See `docs/features/events.md`, "Store admins".
