...
| `app/stores/[slug]/admin/page.tsx` | Store-scoped console (PL-17): store details, its leagues + create league (store fixed), its events + create event (store fixed, paste box included), admin roster | `isStoreAdmin(store.id)` → redirects to `/` if false |
