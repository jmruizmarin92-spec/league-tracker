...
- `app/events/[slug]/display/page.tsx` — venue projector view, the event twin of the session display: the current round of every age division at across-the-hall size, standings beside it, realtime-refreshed, nothing to click. No round clock — TOM runs its own. Shows TOM's official placings once the event is closed, the provisional table before that. Linked from the event header whenever imported rounds exist.
...
- `event-pairings.tsx` — round tabs per age division (the division tab strip only appears when the file has more than one). Round selection is controlled and reconciled during render, not in an effect, so a re-import snaps the room to the new round. Report buttons show only on the viewer's own undecided pairing; a self-report renders as a badge until TOM says otherwise, then disappears.
- `event-my-match.tsx` — the pinned "your match" card: round, table, opponent, and win/draw/lose buttons. Wording is deliberately "has reportado que ganaste", never "ganaste" — TOM has the last word.
