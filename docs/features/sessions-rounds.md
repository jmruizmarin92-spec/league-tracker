...
- `participants-list.tsx` — the admin roster. Each row is `[check-in switch] [name] [archetype chips] [edit archetypes + remove]`; the switch is unlabelled (the label survives only as `aria-label`) and the archetype picker expands full width below its row when opened. The list owns the check-in state for its rows, so the optional "solo pendientes de check-in" filter (registered list only) reacts immediately when a switch is flipped, without waiting for a server round-trip. The waitlist renders a second instance with no filter. Note the header tally (`checkedInCount`) is server-rendered and only updates on revalidation, so it can lag the switches by a moment.
